...
/**
 * Durante la reproducción, el mapa sigue al coche. Sin esto la animación
 * podía estar funcionando y pasar totalmente desapercibida: el marcador es
 * pequeño y con el mapa alejado no se aprecia que se mueva.
 */
function FollowVehicleWhilePlaying() {
  ...
  useEffect(() => {
    ...
    if (!startedRef.current) {
      ...
      map.flyTo([position.latitude, position.longitude], Math.max(map.getZoom(), 11), { duration: 0.6 });
      return;
    }
...
function PlaybackControls({ dayId }: { dayId: string }) {
  ...
  const playTap = useTap(() => (isPlaying ? pause() : playToNextStop()));
  ...
  return (
    <div className="pointer-events-none absolute bottom-[calc(128px+env(safe-area-inset-bottom)+12px)] left-0 right-0 z-[550] flex items-center justify-between gap-2 px-4">
      <div className="flex gap-2">
        <button
          {...playTap}
          className="pointer-events-auto flex h-12 w-12 touch-manipulation items-center justify-center rounded-full bg-(--color-navigation) text-white shadow-(--shadow-card) transition-transform active:scale-95"
          aria-label={isPlaying ? "Pausar" : "Avanzar a la siguiente parada"}
        >
